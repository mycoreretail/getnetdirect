(function(){
  ...
  document.addEventListener('submit',async e=>{const f=e.target.closest('.employee-lead-form');if(!f)return;e.preventDefault();try{await patchSharedLead(f.dataset.leadId,formData(f));await syncSharedData();renderEmployee();toast('Lead update saved')}catch(err){toast(err.message||'Lead update failed')}});
