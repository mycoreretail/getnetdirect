(function(){
  ...
  function apiLeadToLocal(r,session){
    ...
    return {id:r.id,firstName:r.first_name??r.firstName??'',lastName:r.last_name??r.lastName??'',phone:r.phone||'',email:r.email||'',address:r.address||'',unit:r.unit||'',city:r.city||'',state:r.state||'',zip:r.zip||'',services:Array.isArray(r.services)?r.services:[],currentProvider:r.current_provider??r.currentProvider??'',moveIn:r.move_in??r.moveIn??'',contactTime:r.contact_time??r.contactTime??'',notes:r.notes||'',source:r.source_code??r.source??(session?.partnerCode||'Direct'),partnerCode:r.partner_code??r.partnerCode??(partner?(session?.partnerCode||''):'') ,assignedEmployee:r.employee_id??r.employeeId??'',status:normalizeStatus(r.status),priority:r.priority||'Normal',nextFollowUp:r.next_follow_up??r.nextFollowUp??'',provider:r.provider||'',orderNumber:r.order_number??r.orderNumber??'',installDate:r.install_date??r.installDate??'',internalNotes:r.internal_notes??r.internalNotes??'',payoutAmount:r.payout_amount??r.payoutAmount??0,payoutStatus:r.payout_status??r.payoutStatus??'Not Eligible',payoutPaidDate:r.payout_paid_date??r.payoutPaidDate??'',payoutReference:r.payout_reference??r.payoutReference??'',createdAt:r.created_at??r.createdAt??'',updatedAt:r.updated_at??r.updatedAt??'',_partner:partner};
  }
  ...
  async function syncSharedData(){
    ...
    try{
      const lr=await API.leads(); const mapped=(lr.leads||[]).map(r=>apiLeadToLocal(r,session));
      if(session.role==='partner'){mapped.forEach(x=>{x._partner=true;x.source=session.partnerCode||x.source;x.partnerCode=session.partnerCode||x.partnerCode});put(STORE_REF,mapped);put(STORE_DIRECT,[])}
      else{put(STORE_REF,mapped.filter(x=>x._partner));put(STORE_DIRECT,mapped.filter(x=>!x._partner))}
      if(session.role==='admin'){
        const [er,pr]=await Promise.all([API.employees(),API.partners()]);put(STORE_EMP,(er.employees||[]).map(employeeApiToLocal));put(STORE_PARTNERS,(pr.partners||[]).map(partnerApiToLocal));
      }else if(session.role==='employee'&&session.employeeId){put(STORE_EMP,[{id:session.employeeId,name:session.name,email:'',phone:'',role:'Employee / Contractor Sales Rep',code:session.employeeCode||'',status:'Active'}])}
      else if(session.role==='partner'&&session.partnerId){put(STORE_PARTNERS,[{id:session.partnerId,name:session.name,partnerType:'Referral Partner',code:session.partnerCode||'',status:'Active'}])}
      return true;
    }catch(e){console.error('Shared portal sync failed',e);toast('Could not sync shared data');return false}
  }
  ...
  /* Public lead forms */
  async function submitPublic(form,full=false){if(!form.reportValidity())return;const d=formData(form);d.referralCode=d.referralCode||ref||'';try{const r=await API.createPublicLead(d);const rid=r?.lead?.id||'submitted';setStatus(form,`Thank you — request ${rid} was received. A GetNetDirect representative can follow up using the contact information provided.`);form.reset();$$('[data-ref-field]').forEach(el=>{if(ref)el.value=ref});wireServiceChoices(form)}catch(e){setStatus(form,'We could not submit your request right now. Please try again or contact GetNetDirect.','error')}}
  ...
  $('#referralForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=e.currentTarget;if(!f.reportValidity())return;const d=formData(f);d.referralCode=d.partnerCode||partnerCode;try{const r=await API.createPublicLead(d);setStatus(f,`Referral ${r?.lead?.id||''} submitted successfully.`);f.reset();$('#partnerCodeField').value=partnerCode;wireServiceChoices(f);await syncSharedData();renderPartner()}catch(err){setStatus(f,err.message||'Referral could not be submitted.','error')}});
  ...
  $('#adminLeadSearch')?.addEventListener('input',renderAdmin);$('#adminLeadFilter')?.addEventListener('change',renderAdmin);$('#refreshAdmin')?.addEventListener('click',async()=>{await syncSharedData();renderAdmin();toast('Dashboard refreshed')});
  ...
  document.addEventListener('submit',async e=>{const f=e.target.closest('.admin-lead-form');if(!f)return;e.preventDefault();const d=formData(f);if(d.payoutStatus==='Paid'&&!d.payoutPaidDate)d.payoutPaidDate=new Date().toISOString().slice(0,10);try{await patchSharedLead(f.dataset.leadId,d);await syncSharedData();renderAdmin();toast('Lead updated')}catch(err){toast(err.message||'Lead update failed')}});
  ...
  if(window.GNDAuth?.getSession?.()&&API?.token?.()){syncSharedData().then(()=>{renderPartner();renderAdmin();renderEmployee()})}
})();
