(function(){
  ...
  function referralUrl(code){return window.GNDMarketing?.referralUrl?.(code)||`https://getnetdirect.com/?ref=${encodeURIComponent(code||'')}`}
  ...
  /* Public lead forms */
  function submitPublic(form,full=false){if(!form.reportValidity())return;const d=formData(form);d.id=id('GND');d.createdAt=new Date().toISOString();d.status='New';d.priority='Normal';d.source=d.referralCode||ref||'Direct';d.payoutStatus='Not Eligible';const partner=getPartners().find(x=>x.code===d.source),rep=getEmployees().find(x=>x.code===d.source);if(partner){d.partnerCode=d.source;d.payoutAmount=0;save(STORE_REF,d)}else{if(rep)d.assignedEmployee=rep.id;save(STORE_DIRECT,d)}setStatus(form,`Thank you — request ${d.id} was received. A GetNetDirect representative can follow up using the contact information provided.`);form.reset();$$('[data-ref-field]').forEach(el=>{if(ref)el.value=ref});wireServiceChoices(form)}
  ...
  $('#employeeCreateForm')?.addEventListener('submit',e=>{e.preventDefault();const f=e.currentTarget;if(!f.reportValidity())return;const d=formData(f);d.id=id('EMP');d.code=String(d.code||'').trim().toUpperCase()||codeFromName(d.name,'employee');if(codeExists(d.code)){setStatus(f,'That referral/rep code is already in use. Generate a different code.','error');return}d.createdAt=new Date().toISOString();d.status=d.status||'Active';save(STORE_EMP,d);setStatus(f,`${d.name} added. Rep code: ${d.code}`);f.reset();renderAdmin()});
  $('#partnerCreateForm')?.addEventListener('submit',e=>{e.preventDefault();const f=e.currentTarget;if(!f.reportValidity())return;const d=formData(f);d.id=id('PART');d.code=String(d.code||'').trim().toUpperCase()||codeFromName(d.name,'partner');if(codeExists(d.code)){setStatus(f,'That referral code is already in use. Generate a different code.','error');return}d.createdAt=new Date().toISOString();d.status=d.status||'Active';save(STORE_PARTNERS,d);setStatus(f,`${d.name} added. Referral code: ${d.code}`);f.reset();renderAdmin()});
