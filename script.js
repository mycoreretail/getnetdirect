(function(){
  ...
  function renderEmployees(){const body=$('#employeeTableBody');if(!body)return;const emps=getEmployees(),leads=allLeads();body.innerHTML=emps.length?emps.map(e=>`<tr><td><strong>${esc(e.name)}</strong><br><span class="muted-inline">${esc(e.email||'')}</span></td><td>${esc(e.role||'Sales Rep')}</td><td><span class="lead-source">${esc(e.code||'—')}</span></td><td>${leads.filter(x=>x.assignedEmployee===e.id).length}</td><td>${accountBadge(e.status||'Active')}</td><td><a class="mini-action" href="employee-dashboard.html?employee=${encodeURIComponent(e.id)}">View</a> <button class="mini-action" type="button" data-copy-code-link="${esc(e.code||'')}">Link</button> <a class="mini-action" href="${esc(marketingHref(e,'employee'))}">QR/Flyer</a> <button class="mini-action" type="button" data-manage-account="employees:${esc(e.id)}">Set / Reset Login</button> <button class="mini-action" type="button" data-toggle-employee="${esc(e.id)}">${e.status==='Active'?'Deactivate':'Activate'}</button></td></tr>`).join(''):'<tr><td colspan="6" class="empty-state">No sales reps created yet.</td></tr>'}
  function renderPartners(){const body=$('#partnerTableBody');if(!body)return;const ps=getPartners(),refs=arr(STORE_REF);body.innerHTML=ps.length?ps.map(p=>`<tr><td><strong>${esc(p.name)}</strong><br><span class="muted-inline">${esc(p.partnerType||'Referral Partner')}</span></td><td><span class="lead-source">${esc(p.code)}</span></td><td>${refs.filter(r=>(r.source||r.partnerCode)===p.code).length}</td><td>${accountBadge(p.status||'Active')}</td><td><a class="mini-action" href="partner-dashboard.html?partner=${encodeURIComponent(p.code)}">View</a> <button class="mini-action" type="button" data-copy-code-link="${esc(p.code)}">Link</button> <a class="mini-action" href="${esc(marketingHref(p,'partner'))}">QR/Flyer</a> <button class="mini-action" type="button" data-manage-account="partners:${esc(p.id)}">Set / Reset Login</button> <button class="mini-action" type="button" data-partner-status="${esc(p.id)}">${p.status==='Active'?'Deactivate':'Activate'}</button></td></tr>`).join(''):'<tr><td colspan="5" class="empty-state">No referral partners created yet.</td></tr>'}
